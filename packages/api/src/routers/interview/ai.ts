...
export const interviewAiRouter = router({
  ...
  generateGuide: permissionProcedure('interview', 'read')
    ...
    .mutation(({ ctx, input }) =>
      interviewAiService.generateGuide(ctx.user.organizationId, input.interviewId),
    ),
  ...
  generateSummary: permissionProcedure('interview', 'create')
    .input(interviewIdInput)
    .mutation(({ ctx, input }) =>
      interviewAiService.generateSummary(ctx.user.organizationId, input.interviewId),
    ),
  ...
  detectBias: permissionProcedure('interview', 'read')
    .input(interviewIdInput)
    .mutation(({ ctx, input }) =>
      interviewAiService.detectBias(ctx.user.organizationId, input.interviewId),
    ),
});
