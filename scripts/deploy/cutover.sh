...
surface_row() {
  case "$1" in
    ...
    compensation)
      ...
      ;;
    nine-box)
      echo "read|NineBoxReadEnabled|NONE|NONE|NEXT_PUBLIC_NINEBOX_READ_VIA_CSHARP|TS_DELETED|Runbook §6 Phase A #4. UPDATE 2026-07-29: flag confirmed live in prod; 7 of 11 registered read procedures had their TS side deleted. UPDATE 2026-08-05 (#57): the last 4 (getAxisBreakdown, getMovementHistory, simulate, getQuadrantPlan — all zero-FE-consumer) are now deleted too, and with them the whole TS router (packages/api/src/routers/ninebox.ts + .schemas.ts + .helpers.ts are GONE, unregistered from root.ts). scripts/parity/surfaces.ts's 'ninebox' entry was therefore REMOVED and --verify-only for this surface is now a NO-OP, exactly like succession/reporting/evaluation360/team-intel/billing-usage. This row flipped CONFIRMED_LIVE → TS_DELETED and verify → NONE: the previous note explicitly said 'do not treat this as TS_DELETED', which is no longer true. Do NOT read a green run as evidence about the C# read surface — that coverage now lives in NineBoxReadTests.cs / NineBoxReadEndpointAuthTests.cs. NOTE: the WRITE surface is unaffected — see the ninebox-write row below."
      ;;
    engagement)
