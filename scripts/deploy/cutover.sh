...
surface_row() {
  case "$1" in
    ...
    fit-engine)
      ...
      ;;
    fit-engine-write)
      echo "write|FitEngineWriteEnabled|NONE|NONE|NONE|BLOCKED|Phase-5 Slice 24 (issue #90). The 2 FIT-engine writes are deployed DARK behind this flag (POST compute, POST weight-profiles). This flag IS the one-active-writer control for fit_scores + role_family_weight_profiles (both efcoreStranglerWrite) — with it off the routes are never mapped and TS is the sole writer. parity_command NONE for the same reason as the read row above, with one EXTRA fixture hazard specific to the write side: computeForVacancy MUTATES fit_scores, so its write-surface fixture needs a DEDICATED vacancy that no read expectation depends on — this slice's own integration fixture hit exactly that and had to split VacRead from VacInTeam. Both upserts are atomic raw INSERT … ON CONFLICT DO UPDATE (the same statement Prisma emits), so a flip is canary-safe in the byte-identical-rows sense, but do NOT flip before the read row's parity gap is closed. fe_flag NONE — see the read row."
      ;;
    dashboard)
