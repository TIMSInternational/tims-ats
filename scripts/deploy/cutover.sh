...
surface_row() {
  case "$1" in
    ...
    engagement)
      ...
      ;;
    dei)
      echo "read|DeiReadEnabled|verify|dei|NEXT_PUBLIC_DEI_READ_VIA_CSHARP|CONFIRMED_LIVE|Runbook §6 Phase A #4. UPDATE 2026-07-31: flag confirmed live in prod; 8 of 10 registered read procedures (all but getEthnicityDistribution/getDisabilityDistribution, which have zero FE consumers) have ALSO had their TS side deleted — scripts/parity/surfaces.ts's 'dei' entry now registers only those 2 zero-consumer procedures' endpoints. --verify-only still runs a REAL (smaller) check, unlike reporting/evaluation360/team-intel/billing-usage's now-fully-no-op surfaces — do not treat this as TS_DELETED. getPayEquity (FX) was gated by the separate Platform:FxReadsEnabled backend flag but shared this ONE FE flag, so its TS side was deleted in the same pass as the other 8 despite that backend split."
      ;;
    audit-log)
