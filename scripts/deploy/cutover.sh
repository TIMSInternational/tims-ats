...
surface_row() {
  case "$1" in
    team-intel)
      echo "read|TeamIntelReadEnabled|verify|team-intel|NEXT_PUBLIC_TEAMINTEL_READ_VIA_CSHARP|TS_DELETED|UPDATE 2026-08-17 (#195): the read surface was RE-REGISTERED C#-only in scripts/parity/surfaces.ts, so --verify-only is a REAL check again ([WEAK] parity, no TS side; RBAC denies + RLS/IDOR probes are live assertions). Runbook intro + §6 Phase A #1. UPDATE 2026-07-29: the TS getDashboardKpis procedure (packages/api/src/routers/teamIntel.ts) and its FE tRPC fallback (apps/web/lib/platform-api/team-intel.ts) have been deleted — the C# read path is the sole implementation now, so scripts/parity/surfaces.ts's 'team-intel' entry was removed too and there is no TS side left to diff against. --verify-only for this surface was a no-op from that deletion until 2026-08-17 (#195 re-registration; see the UPDATE at the head of this note). UPDATE 2026-08-06 (#55): this row's previous NOTE — 'teamIntel.ts's other 6 procedures (getTeamProfile, getMembers, getBalanceScore, getBalanceAlerts, getRecommendedHires, compareTeams) are untouched' — is now STALE. All 6 have been TS-deleted and packages/api/src/routers/teamIntel.ts is GONE (unregistered from root.ts). The rest of that NOTE stood up to re-verification and still holds: they had zero FE consumers, and they were never part of the C# cutover in the parity sense — the 'team-intel' surface removed above registered exactly ONE endpoint, dashboard-kpis, so the other 6 were NEVER TS-vs-C# parity-diffed by this harness. They DO each have a live C# equivalent (TeamIntelReadEndpoints.cs:38-280 maps all 7 Slice-6 reads) covered by real HTTP integration tests in Tims.IntegrationTests/TeamIntel/. GAP, stated plainly: because the surface was removed on 2026-07-28, those 6 deployed C# endpoints get NO RLS Mode-A IDOR probe and NO RBAC deny assertion from this harness — the same coverage regression #57 caught for nine-box and fixed by retaining a C#-only surface (tsProcedure is optional now). Restoring a C#-only 'team-intel' surface is NOT done here; tracked separately."
      ;;
    reporting)
      echo "read|ReportingReadEnabled|verify|reporting|NEXT_PUBLIC_REPORTING_READ_VIA_CSHARP|TS_DELETED|UPDATE 2026-08-17 (#195): the read surface was RE-REGISTERED C#-only in scripts/parity/surfaces.ts, so --verify-only is a REAL check again ([WEAK] parity, no TS side; RBAC denies + RLS/IDOR probes are live assertions). Runbook §6 Phase A #2. UPDATE 2026-07-28: the TS recruitment-analytics router (packages/api/src/routers/recruitment-analytics.ts) and its FE tRPC fallback (apps/web/lib/platform-api/reporting.ts) have been deleted — the C# read path is the sole implementation now, so scripts/parity/surfaces.ts's 'reporting' entry was removed too and there is no TS side left to diff against. --verify-only for this surface was a no-op from that deletion until 2026-08-17 (#195 re-registration; see the UPDATE at the head of this note)."
      ;;
    billing-read)
    ...
    billing-usage)
      ...
      ;;
    evaluation360)
      echo "read|Evaluation360ReadEnabled|verify|evaluation360|NEXT_PUBLIC_EVALUATION360_READ_VIA_CSHARP|TS_DELETED|UPDATE 2026-08-17 (#195): the read surface was RE-REGISTERED C#-only in scripts/parity/surfaces.ts, so --verify-only is a REAL check again ([WEAK] parity, no TS side; RBAC denies + RLS/IDOR probes are live assertions). Runbook §6 Phase A #4. UPDATE 2026-07-28: the TS evaluation360 router (packages/api/src/routers/evaluation360.ts) and its FE tRPC fallback (apps/web/lib/platform-api/evaluation360.ts, both read AND write) have been deleted — the C# read path is the sole implementation now, so scripts/parity/surfaces.ts's 'evaluation360' entry was removed too and there is no TS side left to diff against for reads. --verify-only for this surface was a no-op from that deletion until 2026-08-17 (#195 re-registration; see the UPDATE at the head of this note). NOTE: the WRITE surface is unaffected by this — see the evaluation360-write row below; scripts/parity/write-surfaces.ts still registers 'evaluation360' for verify-write."
      ;;
    succession)
      echo "read|SuccessionReadEnabled|verify|succession|NEXT_PUBLIC_SUCCESSION_READ_VIA_CSHARP|TS_DELETED|UPDATE 2026-08-17 (#195): the read surface was RE-REGISTERED C#-only in scripts/parity/surfaces.ts, so --verify-only is a REAL check again ([WEAK] parity, no TS side; RBAC denies + RLS/IDOR probes are live assertions). Runbook §6 Phase A #4. UPDATE 2026-07-29: flag confirmed live in prod; 8 of 9 registered read procedures (all but getCriticalRole, which has zero FE consumers) had their TS side deleted. UPDATE 2026-08-03 (#58): getCriticalRole — the 9th and last — is now deleted too, and with it the whole TS router (packages/api/src/routers/succession.ts is GONE, unregistered from root.ts). scripts/parity/surfaces.ts's 'succession' entry was therefore REMOVED and --verify-only for this surface was a NO-OP from that deletion until 2026-08-17 (#195 re-registration; see the UPDATE at the head of this note), exactly like reporting/evaluation360/team-intel/billing-usage. Historical: on 2026-08-03 this row flipped CONFIRMED_LIVE → TS_DELETED and verify → NONE (the previous note's 'do not treat this as TS_DELETED' stopped being true then); the NONE half was reversed on 2026-08-17. Read a green run as evidence about PERMISSIONS and ISOLATION on the C# read surface, NOT as cross-stack parity — [WEAK], no TS side (2026-08-17 re-registration; the dei row's reading applies verbatim). NOTE: the WRITE surface is unaffected — see the succession-write row below."
      ;;
    compensation)
    ...
    access-review)
      ...
      ;;
    fit-engine)
      echo "read|FitEngineReadEnabled|NONE|NONE|NONE|BLOCKED|Phase-5 Slice 24 (issue #90). The 4 FIT-engine reads are deployed DARK behind this flag (ranking, simulate-weights, weight-profiles, explain-fit). parity_command is NONE because scripts/parity/surfaces.ts registers NO fit-engine surface: the harness seed has no fit_engine grants and no fit_scores rows, so --verify-only is not merely Federico-gated, it is UNRUNNABLE BY ANYONE. All six routes therefore sit on the tests/governance/parity-registry-covers-deployed-routes.ts allowlist under two documented groups. To make this row real: seedFitEngineGrants (copy scopes from packages/db/prisma/seed-access-matrix.ts — hr_admin/recruiter@organization, hrbp@unit, leader@team; NEVER invent them, that is #166's false 12/43 FAIL), seed fit_scores rows on a parity vacancy (or ranking/simulate compare empty-vs-empty, a PASS proving nothing), then set this row's parity_command to verify and its parity_key to fit-engine. Tracked on #90. fe_flag is NONE because step 4's FE half is NOT shipped: apps/web/lib/platform-api/fit-engine.ts does not exist. All six procedures have live apps/web tRPC call sites, and explainFit CANNOT get a dual-path hook — the C# answers 501 by design (no Bedrock plane), so flipping a wrapper for it would break the narrative panel. The wrapper is five hooks plus a pinned deliberate omission, its own PR by the #241 precedent."
      ;;
    notification)
      echo "read|NotificationReadEnabled|NONE|NONE|NONE|BLOCKED|Phase-5 Slice 25 (issue #98). The 3 notification reads are deployed DARK behind this flag (GET /notifications, /notifications/unread-count, /notifications/preferences). parity_command is NONE because scripts/parity/surfaces.ts registers NO notification surface, and unlike most unregistered surfaces this one CANNOT be registered as-is: all three procedures are bare protectedProcedure, so they authorize on IDENTITY with no grant at all, and the harness compares by ROLE. A notification surface needs per-role fixture ROWS (each probe role owning its own notifications), not a grant fixture, which is a different and larger job than seedXGrants. It is therefore UNRUNNABLE BY ANYONE today, not Federico-gated. fe_flag is NONE because step 4's FE half is NOT shipped: apps/web/lib/platform-api/notification.ts does not exist. Seven of the eleven procedures have live apps/web tRPC call sites (navbar/notification-dropdown.tsx and platform/notifications/page.tsx); getPreferences, updatePreferences, create and bulkCreate have ZERO FE consumers. The wrapper is its own PR by the #241 precedent. BLOCKING DIVERGENCE for step 5/6, decided by Federico 2026-08-19: notifications RLS is organization_id = app.current_org_id, but rows are addressed by user_id and lib/notify.ts stamps the TARGET org while addressing platform owners — so C# under TenantScope hides rows TS (BYPASSRLS) returns. Measured the same day: 0 rows in notifications and notification_preferences in prod, so it is latent. Flipping this flag before that is resolved would empty the platform notifications page."
      ;;
    notification-write)
      echo "write|NotificationWriteEnabled|NONE|NONE|NONE|BLOCKED|Phase-5 Slice 25 (issue #98). The 8 notification writes are deployed DARK behind this flag: six self-service (POST /notifications/{id}/read, /notifications/read-all, /notifications/{id}/archive, /notifications/archive-read, DELETE /notifications/{id}, PATCH /notifications/preferences) and two grant-gated (POST /notifications, /notifications/bulk, both notification:create). With it off the routes are never mapped. parity_command NONE and fe_flag NONE for the same reasons as the read row. NOTE this flag is the one-active-writer control for the ROUTER path ONLY and an unqualified claim would be false: packages/api/src/lib/notify.ts createMany-s into notifications from routers/platform/organizations.ts:220 and :300 outside any flag, and the C# PlatformOrganizations write/create contexts also INSERT it — that coexistence is why notifications sits in efcoreStranglerWrite and not efcore. NOTE ALSO that the READ flag alone can write notification_preferences, because getPreferences is a tRPC query that INSERTs the caller row on a miss; this write flag is not the only path to that table."
      ;;
    fit-engine-write)
      echo "write|FitEngineWriteEnabled|NONE|NONE|NONE|BLOCKED|Phase-5 Slice 24 (issue #90). The 2 FIT-engine writes are deployed DARK behind this flag (POST compute, POST weight-profiles). With it off the routes are never mapped. It is the one-active-writer control for the ROUTER path ONLY — NOT for the two tables: candidateAiService.screenCandidate (candidate-ai.service.ts:112) upserts fit_scores via computeFitScore and can bootstrap the Default weight profile, and candidateRepository.merge (candidate.repository.ts:458) DELETES fit_scores rows; both are outside any flag and are step-6 preconditions. parity_command NONE for the same reason as the read row above, with one EXTRA fixture hazard specific to the write side: computeForVacancy MUTATES fit_scores, so its write-surface fixture needs a DEDICATED vacancy that no read expectation depends on — this slice's own integration fixture hit exactly that and had to split VacRead from VacInTeam. Both upserts are atomic raw INSERT … ON CONFLICT DO UPDATE (the same statement Prisma emits), so a flip is canary-safe in the byte-identical-rows sense, but do NOT flip before the read row's parity gap is closed. fe_flag NONE — see the read row."
      ;;
    dashboard)
...
EXAMPLES
  ...
  scripts/deploy/cutover.sh --list
  # NOTE: only "billing-usage" and "billing-read" still have no real --verify-only check — their
  # TS procedures were deleted AND their surfaces were removed from scripts/parity/surfaces.ts,
  # so --verify-only prints a no-op notice and exits 0 rather than erroring. Do not read that 0
  # as a pass. UPDATE 2026-08-17 (#195): "reporting", "evaluation360" (read), "team-intel" and
  # "succession" LEFT this list — re-registered C#-only, --verify-only is a real gate again.
  #
...
# (for "billing-usage"/"billing-read", whose surfaces are unregistered, run_verify's NONE branch
# above returns 0 unconditionally instead of a real pass/fail — "audit-log"/"access-review" left
# that group on 2026-08-11; "reporting"/"evaluation360" read/"team-intel"/"succession" left it on
# 2026-08-17, #195). Once a
