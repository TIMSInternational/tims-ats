...
import { useAuditLogs, useAuditLogsExport } from '../../../../lib/platform-api/audit-log';
...
function getActionColor(action: string) {
  ...
  if (action.includes('create') || action.includes('paid') || action.includes('activat') || action.includes('enabled'))
...
export default function AuditPage() {
  ...
  const fetchAuditLogsExport = useAuditLogsExport();
      ...
      <div className="bg-white rounded-xl shadow-[0_1px_4px_rgba(0,0,0,0.06)] p-4">
        <div className="flex items-center gap-3">
          <div className="flex items-center gap-2 border border-[#EDEDED] rounded-lg px-3 py-2">
            ...
            <input
              ...
              className="text-sm text-[#585858] bg-transparent outline-none"
            />
          ...
          <select
            ...
            className="border border-[#EDEDED] rounded-lg px-3 py-2 text-sm text-[#585858] bg-white"
          >
          ...
          <button
            ...
            className="flex items-center gap-2 px-4 py-2 border border-[#EDEDED] rounded-lg text-sm text-[#585858] hover:bg-[#F6F6F6] transition"
          >
        ...
        {hasActiveFilters && (
          <div className="flex items-center gap-2 mt-3">
            ...
            <button onClick={clearFilters} className="text-xs text-[#8B8B8B] hover:text-[#DD0C15] ml-1">
              {t.audit.clearAll}
            </button>
          </div>
        )}
          ...
          <>
                      ...
                      <tr key={log.id} className="hover:bg-[#FAFAFA]/50">
                        ...
                        <td className="px-5 py-3">
                          <div className="flex items-center gap-2">
                            <div
                              className={`w-6 h-6 rounded-full ${getAvatarColor(actorName)} flex items-center justify-center text-[10px] font-semibold text-white`}
                            >
                        ...
                        <td className="px-5 py-3">
                          <span
                            className={`px-2 py-0.5 rounded-full text-[11px] font-medium ${actionStyle.bg} ${actionStyle.text}`}
                          >
                            {log.action}
                          </span>
                        </td>
                        <td className="px-5 py-3 text-sm text-[#585858]">{log.entity ?? '--'}</td>
                        <td className="px-5 py-3 text-sm text-[#8B8B8B] max-w-[200px] truncate">
                          {log.entityId ?? '--'}
                        </td>
            ...
            <div className="flex items-center justify-between px-5 py-3 border-t border-[#EDEDED]">
              <span className="text-sm text-[#8B8B8B]">
                {t.audit.showing} {total > 0 ? page * limit + 1 : 0}-{Math.min((page + 1) * limit, total)} {t.audit.of}{' '}
              ...
              <div className="flex items-center gap-1">
                <button
                  ...
                  disabled={page === 0}
                  className="w-8 h-8 rounded-lg border border-[#EDEDED] flex items-center justify-center text-[#8B8B8B] hover:bg-[#F6F6F6] disabled:opacity-30"
                >
                  <svg className="w-4 h-4" fill="none" stroke="currentColor" strokeWidth="1.5" viewBox="0 0 24 24">
                ...
                {Array.from({ length: Math.min(totalPages, 5) }).map((_, i) => (
                  <button
                    key={i}
                    onClick={() => setPage(i)}
                    className={`w-8 h-8 rounded-lg text-sm font-medium ${page === i ? 'bg-[#1F114C] text-white' : 'border border-[#EDEDED] text-[#585858] hover:bg-[#F6F6F6]'}`}
                  >
                    {i + 1}
                  </button>
                ))}
