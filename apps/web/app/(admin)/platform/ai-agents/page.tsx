    ...
    <div className="h-full flex flex-col overflow-hidden p-5">
      ...
      <div className="flex items-center gap-3 mb-4 shrink-0">
        ...
        <div className="flex items-center gap-1 bg-white border border-[#EDEDED] rounded-lg p-0.5">
          {([['all', t.aiAgents.filterAll], ['mvp', t.aiAgents.filterMvp], ['post-mvp', t.aiAgents.filterPostMvp]] as const).map(([key, label]) => (
            <button key={key} onClick={() => setCategoryFilter(key)} className={`px-3 h-7 text-[11px] font-medium rounded-md transition-colors ${categoryFilter === key ? 'bg-[#1F114C] text-white' : 'text-[#585858] hover:bg-[#F5F5F5]'}`}>{label}</button>
          ))}
        </div>
      ...
      >
        {filteredAgents.map(agent => {
          ...
          return (
            <tr key={agent.id} className="hover:bg-[#FAFAFA] transition-colors border-b border-[#F3F3F3]">
              ...
              <td className="px-4 py-3">
                <div className="flex items-center gap-1">
                  ...
                  <button onClick={() => setEditingAgent(agent)} className="px-2 py-1 text-[10px] rounded font-medium text-[#1F114C] bg-[#F0EEF7] hover:bg-[#E4E0F0] transition-colors">{t.aiAgents.edit}</button>
                </div>
              </td>
            </tr>
          );
      ...
      {editingAgent && (
        <AgentEditDrawer
          ...
          onSuccess={() => { setEditingAgent(null); invalidateAll(); }}
        />
      )}
    </div>
  );
