...
export default function PlatformAiAgentsPage() {
  ...
  const updateAgent = trpc.platform.updateAiAgent.useMutation({
    onSuccess: () => {
      ...
      toast('Estado de agente actualizado', { type: 'success' });
    },
        ...
        <div className="bg-white rounded-xl shadow-[0_1px_4px_rgba(0,0,0,0.06)] p-4">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-lg bg-[#1F114C]/10 flex items-center justify-center">
              <svg className="w-5 h-5 text-[#1F114C]" fill="none" stroke="currentColor" strokeWidth="1.5" viewBox="0 0 24 24"><path d="M9.813 15.904L9 18.75l-.813-2.846a4.5 4.5 0 00-3.09-3.09L2.25 12l2.846-.813a4.5 4.5 0 003.09-3.09L9 5.25l.813 2.846a4.5 4.5 0 003.09 3.09L15.75 12l-2.846.813a4.5 4.5 0 00-3.09 3.09z" /></svg>
            </div>
            <div>
        ...
        <div className="bg-white rounded-xl shadow-[0_1px_4px_rgba(0,0,0,0.06)] p-4">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-lg bg-green-50 flex items-center justify-center">
              <svg className="w-5 h-5 text-green-600" fill="none" stroke="currentColor" strokeWidth="1.5" viewBox="0 0 24 24"><path d="M9 12.75L11.25 15 15 9.75M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
            </div>
            <div>
        ...
        <div className="bg-white rounded-xl shadow-[0_1px_4px_rgba(0,0,0,0.06)] p-4">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-lg bg-amber-50 flex items-center justify-center">
              <svg className="w-5 h-5 text-amber-600" fill="none" stroke="currentColor" strokeWidth="1.5" viewBox="0 0 24 24"><path d="M14.25 9.75L16.5 12l-2.25 2.25m-4.5 0L7.5 12l2.25-2.25M6 20.25h12A2.25 2.25 0 0020.25 18V6A2.25 2.25 0 0018 3.75H6A2.25 2.25 0 003.75 6v12A2.25 2.25 0 006 20.25z" /></svg>
            </div>
            <div>
      ...
      <div className="flex items-center gap-3 mb-4 shrink-0">
        ...
        <div className="flex items-center gap-1 bg-white border border-[#EDEDED] rounded-lg p-0.5">
          {[{ key: 'all', label: 'Todos' }, { key: 'mvp', label: 'MVP' }, { key: 'post-mvp', label: 'Post-MVP' }].map(tab => (
            <button key={tab.key} onClick={() => setCategoryFilter(tab.key)} className={`px-3 h-7 text-[11px] font-medium rounded-md transition-colors ${categoryFilter === tab.key ? 'bg-[#1F114C] text-white' : 'text-[#585858] hover:bg-[#F5F5F5]'}`}>{tab.label}</button>
          ))}
        </div>
        <select value={statusFilter} onChange={e => setStatusFilter(e.target.value)} className="h-9 px-3 text-[12px] border border-[#EDEDED] rounded-lg bg-white text-[#585858] focus:outline-none">
        ...
        {isEmpty && (
          <button onClick={() => seedAgents.mutate()} disabled={seedAgents.isPending} className="ml-auto h-9 px-4 text-[12px] font-medium bg-[#1F114C] text-white rounded-lg hover:bg-[#1F114C]/90 disabled:opacity-50 transition-colors">
      ...
      <div className="flex-1 bg-white rounded-xl shadow-[0_1px_4px_rgba(0,0,0,0.06)] overflow-hidden flex flex-col min-h-0">
        {isEmpty ? (
          <div className="flex-1 flex items-center justify-center">
            <div className="text-center">
              <svg className="w-12 h-12 mx-auto mb-3 text-[#ccc]" fill="none" stroke="currentColor" strokeWidth="1.5" viewBox="0 0 24 24"><path d="M9.813 15.904L9 18.75l-.813-2.846a4.5 4.5 0 00-3.09-3.09L2.25 12l2.846-.813a4.5 4.5 0 003.09-3.09L9 5.25l.813 2.846a4.5 4.5 0 003.09 3.09L15.75 12l-2.846.813a4.5 4.5 0 00-3.09 3.09z" /></svg>
                ...
                {filteredAgents.map(agent => {
                  ...
                  return (
                    <tr key={agent.id} className="hover:bg-[#FAFAFA] transition-colors">
                      ...
                      <td className="px-4 py-3"><span className="text-[12px] text-[#585858]">{formatCache(agent.cacheTtlSeconds)}</span></td>
                      <td className="px-4 py-3 text-right"><span className="text-[12px] font-mono text-[#585858]">{formatCost(agent.costPerCall)}</span></td>
                      <td className="px-4 py-3 text-center">
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-1">
                          ...
                          <button onClick={() => setEditingAgent(agent)} className="px-2 py-1 text-[10px] rounded font-medium text-[#1F114C] bg-[#F0EEF7] hover:bg-[#E4E0F0] transition-colors">Editar</button>
                        </div>
                      </td>
                    </tr>
                  );
        ...
        {!isEmpty && (
          <div className="border-t border-[#EDEDED] px-4 py-2 flex items-center justify-between shrink-0">
            <p className="text-[11px] text-[#8B8B8B]">Mostrando {filteredAgents.length} de {agents?.length ?? 0} agentes</p>
          </div>
        )}
      </div>
