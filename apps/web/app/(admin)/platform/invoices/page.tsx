...
function statusBadge(status: string, dueDate?: Date | string | null) {
  const isOverdue = status === 'pending' && dueDate && new Date(dueDate) < new Date();
  if (isOverdue) return <span className="px-2 py-1 rounded-full text-[10px] font-bold bg-red-100 text-red-700">Vencida</span>;
  const map: Record<string, { cls: string; label: string }> = {
    draft: { cls: 'bg-gray-100 text-gray-600', label: 'Borrador' },
    pending: { cls: 'bg-amber-100 text-amber-700', label: 'Pendiente' },
    paid: { cls: 'bg-green-100 text-green-700', label: 'Pagada' },
    void: { cls: 'bg-gray-100 text-gray-500', label: 'Anulada' },
  };
    ...
    <div className="h-full flex flex-col overflow-hidden p-6">
      ...
      <div className="grid grid-cols-4 gap-4 mb-5 flex-shrink-0">
        {kpis.isLoading ? Array.from({ length: 4 }).map((_, i) => (
          ...
          <div className="bg-white rounded-xl shadow-[0_1px_4px_rgba(0,0,0,0.06)] p-5">
            <div className="flex items-center justify-between mb-3"><span className="text-xs text-[#8B8B8B] font-medium uppercase tracking-wide">Recaudado este mes</span><div className="w-8 h-8 rounded-lg bg-green-50 flex items-center justify-center"><svg className="w-4 h-4 text-green-500" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path d="M12 1v22M17 5H9.5a3.5 3.5 0 100 7h5a3.5 3.5 0 110 7H6" /></svg></div></div>
            <div className="text-2xl font-bold text-[#333]">{fmtCurrency(kpis.data.collected)}</div><div className="text-xs text-green-500 mt-1 font-medium">Facturas pagadas</div>
          </div>
          <div className="bg-white rounded-xl shadow-[0_1px_4px_rgba(0,0,0,0.06)] p-5">
            <div className="flex items-center justify-between mb-3"><span className="text-xs text-[#8B8B8B] font-medium uppercase tracking-wide">Pendiente</span><div className="w-8 h-8 rounded-lg bg-amber-50 flex items-center justify-center"><svg className="w-4 h-4 text-amber-500" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><circle cx="12" cy="12" r="10" /><path d="M12 6v6l4 2" /></svg></div></div>
            <div className="text-2xl font-bold text-amber-600">{fmtCurrency(kpis.data.outstanding)}</div><div className="text-xs text-amber-500 mt-1 font-medium">Por cobrar</div>
          </div>
          <div className={`bg-white rounded-xl shadow-[0_1px_4px_rgba(0,0,0,0.06)] p-5 ${kpis.data.overdueCount > 0 ? 'border border-red-200' : ''}`}>
            <div className="flex items-center justify-between mb-3"><span className="text-xs text-[#8B8B8B] font-medium uppercase tracking-wide">Vencidas</span><div className="w-8 h-8 rounded-lg bg-red-50 flex items-center justify-center"><svg className="w-4 h-4 text-[#DD0C15]" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path d="M10.29 3.86L1.82 18a2 2 0 001.71 3h16.94a2 2 0 001.71-3L13.71 3.86a2 2 0 00-3.42 0zM12 9v4M12 17h.01" /></svg></div></div>
            <div className={`text-2xl font-bold ${kpis.data.overdueCount > 0 ? 'text-[#DD0C15]' : 'text-[#333]'}`}>{kpis.data.overdueCount}</div><div className={`text-xs mt-1 font-medium ${kpis.data.overdueCount > 0 ? 'text-[#DD0C15]' : 'text-[#8B8B8B]'}`}>{kpis.data.overdueCount > 0 ? 'Requiere atencion' : 'Sin vencidas'}</div>
          </div>
          <div className="bg-white rounded-xl shadow-[0_1px_4px_rgba(0,0,0,0.06)] p-5">
            <div className="flex items-center justify-between mb-3"><span className="text-xs text-[#8B8B8B] font-medium uppercase tracking-wide">Dias Prom. Pago</span><div className="w-8 h-8 rounded-lg bg-blue-50 flex items-center justify-center"><svg className="w-4 h-4 text-blue-500" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" /></svg></div></div>
            <div className="text-2xl font-bold text-[#333]">{kpis.data.avgDaysToPay}</div><div className="text-xs text-[#8B8B8B] mt-1">dias promedio</div>
          </div>
        </> : null}
      </div>
      ...
      <div className="flex items-center justify-between mb-4 flex-shrink-0">
        ...
        <div className="flex items-center gap-2">
          <div className="relative">
            ...
            <input type="text" value={search} onChange={(e) => { setSearch(e.target.value); setPage(0); }} placeholder="Buscar organizacion..." className="h-8 pl-9 pr-3 rounded-lg border border-[#EDEDED] text-xs text-[#333] placeholder:text-[#8B8B8B] focus:outline-none focus:ring-2 focus:ring-[#1F114C]/20 w-52" />
          </div>
          ...
          <button onClick={() => setShowWizard(true)} className="h-8 px-4 rounded-lg bg-[#1F114C] text-white text-xs font-medium hover:bg-[#2a1866] transition flex items-center gap-1.5">
            <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path d="M12 4v16m8-8H4" /></svg>Nueva Factura
          </button>
        </div>
      </div>
      ...
      <div className="bg-white rounded-xl shadow-[0_1px_4px_rgba(0,0,0,0.06)] flex-1 flex flex-col min-h-0 overflow-hidden">
            ...
            <tbody>
              {invoices.isLoading ? Array.from({ length: 5 }).map((_, i) => (
                ...
                <tr><td colSpan={7} className="px-5 py-16 text-center">
                  ...
                  <p className="text-sm text-[#8B8B8B]">No hay facturas registradas</p>
                  <button onClick={() => setShowWizard(true)} className="mt-3 text-xs text-[#1F114C] font-medium hover:underline">Crear primera factura</button>
                </td></tr>
              ) : rows.map((inv) => {
                ...
                return (
                  <tr key={inv.id} className={`border-b border-[#F6F6F6] hover:bg-[#FAFAFA] transition cursor-pointer ${isVoid ? 'opacity-50' : ''}`} onClick={() => setViewInvoiceId(inv.id)}>
                    ...
                    <td className="px-4 py-3 text-center">{statusBadge(inv.status, inv.dueDate)}</td>
                    ...
                    <td className="px-4 py-3" onClick={(e) => e.stopPropagation()}>
                      <div className="flex items-center justify-center gap-1.5">
                        {!isPaid && !isVoid && <>
                          <button onClick={() => { if (confirm('Marcar como pagada?')) updateStatus.mutate({ id: inv.id, status: 'paid' }); }} disabled={updateStatus.isPending} className="text-[10px] text-green-600 font-medium hover:underline disabled:opacity-50">Pagada</button><span className="text-[#EDEDED]">|</span>
                          <button onClick={() => sendReminder.mutate({ id: inv.id })} disabled={sendReminder.isPending} className="text-[10px] text-blue-600 font-medium hover:underline disabled:opacity-50">Recordar</button><span className="text-[#EDEDED]">|</span>
                        </>}
                        {isPaid && <><button onClick={() => { if (confirm('Revertir a pendiente?')) updateStatus.mutate({ id: inv.id, status: 'pending' }); }} disabled={updateStatus.isPending} className="text-[10px] text-amber-600 font-medium hover:underline disabled:opacity-50">Revertir</button><span className="text-[#EDEDED]">|</span></>}
                        {!isVoid ? <button onClick={() => { if (confirm('Anular factura?')) updateStatus.mutate({ id: inv.id, status: 'void' }); }} disabled={updateStatus.isPending} className="text-[10px] text-[#DD0C15] font-medium hover:underline disabled:opacity-50">Anular</button> : <span className="text-[10px] text-[#8B8B8B]">Anulada</span>}
                      </div>
                    </td>
                  </tr>
                );
        ...
        <div className="flex items-center justify-between px-5 py-3.5 border-t border-[#EDEDED] flex-shrink-0">
          <span className="text-xs text-[#8B8B8B]">Mostrando {rows.length > 0 ? page * limit + 1 : 0}-{page * limit + rows.length} de {total}</span>
...
function InvoiceWizard({ onClose, onSuccess }: { onClose: () => void; onSuccess: () => void }) {
  ...
  const STEPS = ['Cliente', 'Configuracion', 'Detalles', 'Revisar y Enviar'];
    ...
    <div className="h-full flex flex-col bg-[#FAFAFA]">
      ...
      <div className="flex items-center justify-between px-6 py-4 bg-white border-b border-[#EDEDED] flex-shrink-0">
        <div className="flex items-center gap-4">
          ...
          <h1 className="text-lg font-semibold text-[#333]">Nueva Factura</h1>
        </div>
        <div className="flex items-center gap-1">
          {STEPS.map((s, i) => (
      ...
      <div className="flex-1 flex min-h-0 overflow-hidden">
        ...
        <div className="w-1/2 overflow-y-auto p-8 border-r border-[#EDEDED]">
          {step === 0 && (
            <div>
              <h2 className="text-2xl font-semibold text-[#333] mb-2">Seleccionar cliente</h2>
              <p className="text-sm text-[#8B8B8B] mb-6">Busca la organizacion a la que deseas facturar.</p>
              <input type="text" value={orgSearch} onChange={(e) => { setOrgSearch(e.target.value); setOrgId(''); }} placeholder="Buscar organizacion..." className="w-full h-11 px-4 rounded-xl border border-[#EDEDED] text-sm text-[#333] placeholder:text-[#8B8B8B] focus:outline-none focus:ring-2 focus:ring-[#1F114C]/20 mb-2" />
          ...
          {step === 1 && (
            <div>
              <h2 className="text-2xl font-semibold text-[#333] mb-2">Configurar factura</h2>
              ...
              <div className="mb-4">
                <div className="grid grid-cols-[1fr_70px_100px_32px] gap-2 mb-2">
                  <span className="text-xs font-medium text-[#585858]">Item</span>
                  <span className="text-xs font-medium text-[#585858]">Cant.</span>
                  <span className="text-xs font-medium text-[#585858]">Precio unit.</span>
                  <span />
                </div>
                {lineItems.map((li, i) => (
                ...
                <button onClick={addLine} className="text-xs text-[#1F114C] font-medium hover:underline flex items-center gap-1 mt-1"><svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path d="M12 4v16m8-8H4" /></svg>Agregar item</button>
              </div>
              ...
              <div className="mt-4">
                <label className="text-xs font-medium text-[#585858] mb-1 block">Memo para el pagador (opcional)</label>
          ...
          {step === 2 && (
            <div>
              <h2 className="text-2xl font-semibold text-[#333] mb-2">Detalles de factura</h2>
              <p className="text-sm text-[#8B8B8B] mb-6">Numero de factura, fechas y referencia.</p>
              <div className="grid grid-cols-2 gap-4 mb-4">
                <div><label className="text-xs font-medium text-[#585858] mb-1 block">Numero de factura</label><input type="text" value={invNumber} readOnly className="w-full h-10 px-3 rounded-lg border border-[#EDEDED] text-sm text-[#333] bg-[#F6F6F6] font-mono" /></div>
                <div><label className="text-xs font-medium text-[#585858] mb-1 block">No. PO (opcional)</label><input type="text" value={poNumber} onChange={(e) => setPoNumber(e.target.value)} placeholder="PO-12345" className="w-full h-10 px-3 rounded-lg border border-[#EDEDED] text-sm text-[#333] placeholder:text-[#8B8B8B] focus:outline-none focus:ring-2 focus:ring-[#1F114C]/20" /></div>
              </div>
              <div className="grid grid-cols-2 gap-4 mb-6">
                <div><label className="text-xs font-medium text-[#585858] mb-1 block">Fecha de factura</label><input type="date" value={invoiceDate} onChange={(e) => setInvoiceDate(e.target.value)} className="w-full h-10 px-3 rounded-lg border border-[#EDEDED] text-sm text-[#333] focus:outline-none focus:ring-2 focus:ring-[#1F114C]/20" /></div>
                <div><label className="text-xs font-medium text-[#585858] mb-1 block">Fecha de vencimiento</label><input type="date" value={dueDate} onChange={(e) => setDueDate(e.target.value)} className="w-full h-10 px-3 rounded-lg border border-[#EDEDED] text-sm text-[#333] focus:outline-none focus:ring-2 focus:ring-[#1F114C]/20" /></div>
              </div>
              <div className="mb-4">
                <label className="text-xs font-medium text-[#585858] mb-1 block">Moneda</label>
              ...
              <div><label className="text-xs font-medium text-[#585858] mb-1 block">Nota interna (no visible al cliente)</label><textarea value={notes} onChange={(e) => setNotes(e.target.value)} rows={2} placeholder="Solo visible para el equipo..." className="w-full px-3 py-2 rounded-lg border border-[#EDEDED] text-sm text-[#333] placeholder:text-[#8B8B8B] focus:outline-none focus:ring-2 focus:ring-[#1F114C]/20 resize-none" /></div>
            </div>
          )}

          {step === 3 && (
            <div>
              <h2 className="text-2xl font-semibold text-[#333] mb-2">Revisar y enviar</h2>
              <div className="flex items-center justify-between mb-4 p-3 rounded-xl bg-[#F6F6F6]">
                <div><p className="text-xs text-[#8B8B8B]">Factura para</p><p className="text-sm font-semibold text-[#333]">{orgName}</p></div>
      ...
      <div className="flex items-center justify-center gap-3 px-6 py-4 bg-white border-t border-[#EDEDED] flex-shrink-0">
        {step > 0 && <button onClick={() => setStep(step - 1)} className="h-10 px-5 rounded-lg border border-[#EDEDED] text-sm font-medium text-[#585858] hover:bg-[#F6F6F6] transition flex items-center gap-1.5"><svg className="w-4 h-4" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path d="M15 19l-7-7 7-7" /></svg>Atras</button>}
        {step < 3 && <button onClick={() => setStep(step + 1)} disabled={!canNext()} className="h-10 px-6 rounded-lg bg-[#1F114C] text-white text-sm font-medium hover:bg-[#2a1866] transition disabled:opacity-40 flex items-center gap-1.5">Siguiente<svg className="w-4 h-4" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path d="M9 5l7 7-7 7" /></svg></button>}
        {step === 3 && <>
          <button onClick={() => handleSubmit(false)} disabled={createInvoice.isPending} className="h-10 px-5 rounded-lg border border-[#EDEDED] text-sm font-medium text-[#585858] hover:bg-[#F6F6F6] transition disabled:opacity-50">Solo Crear</button>
          <button onClick={() => handleSubmit(true)} disabled={createInvoice.isPending} className="h-10 px-6 rounded-lg bg-[#1F114C] text-white text-sm font-medium hover:bg-[#2a1866] transition disabled:opacity-50 flex items-center gap-1.5">{createInvoice.isPending ? 'Creando...' : 'Crear & Enviar Email'}<svg className="w-4 h-4" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path d="M9 5l7 7-7 7" /></svg></button>
        </>}
      </div>
    </div>
  );
    ...
    <div className="h-full flex flex-col overflow-hidden">
      ...
      <div className="flex items-center justify-between px-6 py-4 bg-white border-b border-[#EDEDED] flex-shrink-0">
        <div className="flex items-center gap-3">
          ...
          <div className="ml-2">{statusBadge(inv.status, inv.dueDate)}</div>
        </div>
        <div className="flex items-center gap-2">
          {!isPaid && !isVoid && <>
      ...
      <div className="flex-1 overflow-y-auto bg-[#F0F0F0] flex justify-center p-8">
        <div className="bg-white rounded-xl shadow-lg p-10 w-full max-w-[700px] min-h-[700px]">
          ...
          <div className="flex justify-end mb-8"><div className="text-right"><span className="text-base font-semibold text-[#333]">Total</span><span className="text-2xl font-bold text-[#333] ml-6">{fmtCurrency(inv.amount, inv.currency)}</span></div></div>
          {/* Terms */}
...
function BillingProfileDrawer({ organizationId, onClose }: { organizationId: string; onClose: () => void }) {
  ...
  const fields: Array<{ key: keyof typeof form; label: string; type?: string; span?: number }> = [
    { key: 'companyName', label: 'Razon Social', span: 2 }, { key: 'taxId', label: 'NIT / Tax ID' }, { key: 'billingEmail', label: 'Email de Facturacion', type: 'email' }, { key: 'billingPhone', label: 'Telefono' }, { key: 'address', label: 'Direccion', span: 2 }, { key: 'city', label: 'Ciudad' }, { key: 'state', label: 'Departamento / Estado' }, { key: 'country', label: 'Pais' }, { key: 'zipCode', label: 'Codigo Postal' },
  ];
  return (
    <div className="fixed inset-0 z-50 flex justify-end"><div className="absolute inset-0 bg-black/40" onClick={onClose} /><div className="relative bg-white w-full max-w-md h-full overflow-y-auto shadow-xl">
      ...
      {profile.isLoading ? <div className="p-5 space-y-4">{Array.from({ length: 6 }).map((_, i) => <div key={i}><Skeleton className="h-3 w-20 mb-1" /><Skeleton className="h-9 w-full" /></div>)}</div> : (
        ...
        <div className="flex justify-end gap-2 mt-6"><button type="button" onClick={onClose} className="h-9 px-4 rounded-lg border border-[#EDEDED] text-sm font-medium text-[#585858] hover:bg-[#F6F6F6] transition">Cancelar</button><button type="submit" disabled={upsert.isPending} className="h-9 px-5 rounded-lg bg-[#1F114C] text-white text-sm font-medium hover:bg-[#2a1866] transition disabled:opacity-50">{upsert.isPending ? 'Guardando...' : 'Guardar'}</button></div></form>
      )}
    </div></div>
  );
}
