...
import {
  useEngagementListActionPlans,
  useEngagementListLeaderCommitments,
} from '../../../../lib/platform-api/engagement';

function statusMeta(t: ReturnType<typeof useI18n>['t'], status: string): { cls: string; label: string } {
  switch (status) {
    case 'in_progress':
      return { cls: 'text-blue-600 bg-blue-50', label: t.climate.statusInProgress };
    case 'completed':
      return { cls: 'text-green-600 bg-green-50', label: t.climate.statusCompleted };
    case 'fulfilled':
      return { cls: 'text-green-600 bg-green-50', label: t.climate.statusFulfilled };
    case 'overdue':
      return { cls: 'text-red-600 bg-red-50', label: t.climate.statusOverdue };
    default:
      return { cls: 'text-amber-600 bg-amber-50', label: t.climate.statusPending };
  }
}
...
export function ActionPlans() {
  const { t } = useI18n();
  const q = useEngagementListActionPlans();
              ...
              {q.data.map((p, i) => {
                ...
                return (
                  <tr key={p.id} className={i < q.data!.length - 1 ? 'border-b border-[#F6F6F6]' : ''}>
                    ...
                    <td className="py-2 text-[#585858]">
                      {p.responsible ? `${p.responsible.firstName} ${p.responsible.lastName}` : '—'}
                    </td>
...
export function LeaderCommitments() {
  const { t } = useI18n();
  const q = useEngagementListLeaderCommitments();
