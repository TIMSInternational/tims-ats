...
export default function TalentPoolsPage() {
  ...
  const updateFilter = useCallback(<K extends keyof TalentPoolFilterState>(
    key: K,
    value: TalentPoolFilterState[K],
  ) => {
    ...
    <div className="h-full flex flex-col overflow-hidden">
      ...
      <div className="flex flex-wrap items-center justify-between gap-y-2 px-4 md:px-6 min-h-16 py-2 bg-white border-b border-[#EDEDED] shrink-0">
        ...
        <div className="flex flex-wrap items-center gap-2 md:gap-3">
          ...
          <button onClick={handleRecontact} className="flex items-center gap-1.5 bg-[#1F114C] text-white px-4 h-8 rounded-lg text-[12px] font-medium">
            <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" strokeWidth="1.5" viewBox="0 0 24 24">
              <path d="M21.75 6.75v10.5a2.25 2.25 0 01-2.25 2.25h-15a2.25 2.25 0 01-2.25-2.25V6.75m19.5 0A2.25 2.25 0 0019.5 4.5h-15a2.25 2.25 0 00-2.25 2.25m19.5 0v.243a2.25 2.25 0 01-1.07 1.916l-7.5 4.615a2.25 2.25 0 01-2.36 0L3.32 8.91a2.25 2.25 0 01-1.07-1.916V6.75" />
            </svg>
            {t.talentPool.recontactCampaign}
          </button>
          <button onClick={() => setShowCreateModal(true)} className="flex items-center gap-1.5 bg-[#DD0C15] text-white px-4 h-8 rounded-lg text-[12px] font-medium">
