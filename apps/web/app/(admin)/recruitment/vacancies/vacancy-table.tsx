...
export function VacancyTable({ vacancies, isLoading, onDuplicate, onClose, onFreeze }: VacancyTableProps) {
  const { t } = useI18n();
  ...
  return (
    <DataTable
      ...
      empty={
        <EmptyState
          icon={<svg className="w-10 h-10 text-[#ccc]" fill="none" stroke="currentColor" strokeWidth="1.5" viewBox="0 0 24 24"><rect x="2" y="7" width="20" height="14" rx="2" /><path d="M16 7V5a4 4 0 00-8 0v2" /></svg>}
    ...
    >
      {vacancies.map((v) => (
        <tr key={v.id} className="border-b border-[#F6F6F6] hover:bg-[#FAFAFA] transition">
          <td className="px-4 py-3">
            <Link href={`/recruitment/vacancies/${v.id}`} className="hover:underline">
              <p className="text-[13px] font-medium text-[#333]">{v.title}</p>
              {v.company && <p className="text-[11px] text-[#8B8B8B]">{v.company.name}</p>}
            </Link>
          </td>
          <td className="px-4 py-3">
          ...
          <td className="px-4 py-3">
            <div className="flex items-center gap-1.5">
              ...
              <span className="text-[12px] text-[#585858]">{priorityLabels[v.priority] ?? v.priority}</span>
            </div>
          </td>
          ...
          <td className="px-4 py-3 text-center">
            <span className="text-[13px] font-medium text-[#1F114C]">{v._count.applications}</span>
          </td>
          <td className="px-4 py-3">
            <span className="text-[12px] text-[#8B8B8B]">{formatDate(v.createdAt)}</span>
          </td>
          <td className="px-4 py-3 text-right">
            <div className="flex items-center justify-end gap-1">
              ...
              >
                <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" strokeWidth="1.5" viewBox="0 0 24 24"><rect x="8" y="8" width="12" height="12" rx="2" /><path d="M16 8V6a2 2 0 00-2-2H6a2 2 0 00-2 2v8a2 2 0 002 2h2" /></svg>
              </button>
              {v.status !== 'closed' && v.status !== 'frozen' && (
                ...
                >
                  <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" strokeWidth="1.5" viewBox="0 0 24 24"><path d="M6 18L18 6M6 6l12 12" /></svg>
                </button>
              )}
              {v.status === 'published' && (
                <button
                ...
                >
                  <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" strokeWidth="1.5" viewBox="0 0 24 24"><path d="M12 2v20M2 12h20M4.93 4.93l14.14 14.14M19.07 4.93L4.93 19.07" /></svg>
                </button>
              )}
            </div>
          </td>
