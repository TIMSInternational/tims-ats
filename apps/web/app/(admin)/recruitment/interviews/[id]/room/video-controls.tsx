...
export function VideoControls() {
  ...
  // Get local participant to check track states
  const localParticipant = daily?.participants()?.local;
  const isCamOff = localParticipant ? !localParticipant.video : true;
  const isMicOff = localParticipant ? !localParticipant.audio : true;
    ...
    <div className="flex items-center justify-center gap-3 py-3 bg-[#111] shrink-0">
      ...
      <button
        ...
        className={`w-10 h-10 rounded-full flex items-center justify-center transition-colors ${
          !isJoined ? 'bg-white/5 cursor-not-allowed' :
      ...
      >
        ...
        {isCamOff && isJoined && <div className="absolute w-6 h-[2px] bg-red-400 rotate-45 rounded" />}
      </button>
      ...
      <button
        ...
        disabled={!isJoined}
        className={`w-10 h-10 rounded-full flex items-center justify-center transition-colors ${
          !isJoined ? 'bg-white/5 cursor-not-allowed' : 'bg-white/10 hover:bg-white/20'
        }`}
      ...
      {/* Chat */}
      <button className="w-10 h-10 rounded-full bg-white/10 hover:bg-white/20 flex items-center justify-center transition-colors" title="Chat">
        <svg className="w-5 h-5 text-white" fill="none" stroke="currentColor" strokeWidth="1.5" viewBox="0 0 24 24">
      ...
      {/* Notes */}
      <button className="w-10 h-10 rounded-full bg-white/10 hover:bg-white/20 flex items-center justify-center transition-colors" title="Notas">
        <svg className="w-5 h-5 text-white" fill="none" stroke="currentColor" strokeWidth="1.5" viewBox="0 0 24 24">
