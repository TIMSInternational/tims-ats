...
export function MeApplicationTimeline({
  orgSlug,
  applicationId,
}: {
  orgSlug: string;
  ...
  if (isLoading) {
    return <p className="text-[12px] text-[#8B8B8B] mt-3">{t.portalMe.timelineLoading}</p>;
  }
  if (isError || !data) {
    return <p className="text-[12px] text-[#B42318] mt-3">{t.portalMe.timelineError}</p>;
  }
  if (data.movements.length === 0) {
    return <p className="text-[12px] text-[#8B8B8B] mt-3">{t.portalMe.timelineEmpty}</p>;
  }
