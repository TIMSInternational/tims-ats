...
export function RegisterForm({ accountType, onBack }: { accountType: AccountType; onBack: () => void }) {
  ...
  if (success) {
    return (
      <div className="text-center">
        ...
        <h3 className="text-[15px] font-semibold text-[#1F114C] mb-2">Verifica tu email</h3>
  ...
  return (
    <>
      ...
      <div className="flex items-center gap-3 mb-6">
        <button onClick={onBack} className="w-8 h-8 rounded-lg flex items-center justify-center hover:bg-[#F6F6F6] transition-colors">
          <svg className="w-4 h-4 text-[#585858]" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
      ...
      <div className="space-y-3 mb-6">
        <button onClick={handleGoogleSignUp} className="w-full flex items-center justify-center gap-3 h-11 rounded-xl border border-[#EDEDED] text-[13px] font-medium text-[#333] hover:bg-[#FAFAFA] transition">
      ...
      <form onSubmit={handleRegister} className="space-y-4">
        ...
        {accountType === 'company' && (
          <div>
            <label className="block text-[12px] font-medium text-[#585858] mb-1.5">Nombre de la empresa</label>
            <input type="text" value={companyName} onChange={(e) => setCompanyName(e.target.value)} required className="w-full h-11 px-4 rounded-xl border border-[#EDEDED] text-[13px] text-[#333] placeholder:text-[#8B8B8B] focus:outline-none focus:ring-2 focus:ring-[#1F114C]/20 focus:border-[#1F114C] transition" placeholder="Tu Empresa S.A.S" />
          </div>
        )}
