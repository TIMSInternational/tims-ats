...
[CollectionDefinition("UserInvitationCreate")]
public sealed class UserInvitationCollection : ICollectionFixture<OrganizationInvitationFixture>;

[Collection("UserInvitationCreate")]
public sealed partial class UserInvitationEndpointTests(OrganizationInvitationFixture fixture)
{
    ...
    private WebApplicationFactory<Program> Factory(FakeSender sender, bool enabled = true, bool replaceSender = true, string? connection = null) =>
        new WebApplicationFactory<Program>().WithWebHostBuilder(builder =>
        {
            ...
            builder.UseSetting("Platform:PlatformUserInvitationCreateEnabled", enabled.ToString());
    ...
    {
        var request = new HttpRequestMessage(HttpMethod.Post, "/platform/invitations/users")
        { Content = new StringContent(body, System.Text.Encoding.UTF8, "application/json") };
        if (sub is not null) request.Headers.Add("Authorization", "Bearer " + Mint(sub));
        return client.SendAsync(request);
    }

    private static string Input(Guid? org = null, string? role = null) => JsonSerializer.Serialize(new
    {
        email = "invitee@example.test",
        organizationId = org ?? PlatformOrganizationsCreateFixture.OtherOrg,
        roleSlug = role,
    ...
    private sealed class FakeSender : IEmailSender
    {
        ...
        { Calls++; Html = html; return Handler(); }
    }
}
