...
[Collection("OrganizationInvitationCreate")]
public sealed partial class OrganizationInvitationEndpointTests(OrganizationInvitationFixture fixture)
{
    ...
    private WebApplicationFactory<Program> Factory(FakeSender sender, bool enabled = true, bool replaceSender = true, string? connection = null) =>
        new WebApplicationFactory<Program>().WithWebHostBuilder(builder =>
        {
            ...
            builder.UseSetting("Platform:PlatformOrganizationInvitationCreateEnabled", enabled.ToString());
    ...
    {
        var request = new HttpRequestMessage(HttpMethod.Post, "/platform/invitations/organizations")
        { Content = new StringContent(body, System.Text.Encoding.UTF8, "application/json") };
        if (sub is not null) request.Headers.Add("Authorization", "Bearer " + Mint(sub));
        return client.SendAsync(request);
    }

    private static string Input(string? slug = null, string plan = "trial") => JsonSerializer.Serialize(new
    {
        email = "admin@example.test",
        organizationName = "Example <script>test</script>",
        organizationSlug = slug ?? "org-" + Guid.NewGuid().ToString("N"),
