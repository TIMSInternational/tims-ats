...
variable "feature_flags" {
  ...
  type = object({
    ...
    proctoring                 = optional(bool, false) # consented local-observation beta; dark by default
    proctoring_media_evidence  = optional(bool, false) # future S3 capture path; requires approved notice
    proctoring_cloud_inference = optional(bool, false) # future SQS/Lambda path; requires evaluated model
  })
  default = {}
}
