...
| 3   | Tests (vitest)                 | `npx vitest run` (expect **2740 passing across 291 files** as of 2026-08-07; more is fine, fewer is a failure). Run it with nothing else going — the suite contends badly and produced 12 phantom failures while a `tsc` ran alongside it. **Never pipe it** (`vitest \| tail` reports `tail`'s exit 0 over a 17-test failure, and zsh has no `PIPESTATUS`); redirect to a file and check `$?`. Bump this when you add tests — a stale anchor makes "fewer is a failure" unenforceable, the same defect class as a gate that cannot fail.                                                           |
