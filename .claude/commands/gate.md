...
| 14  | RLS tenant isolation (live DB) | `npx tsx scripts/security/verify-rls-isolation.ts` → exit 0. **Requires a working `DIRECT_URL`/`DATABASE_URL`** (session pooler, :5432). If no DB is reachable, report this check as ⚠️ SKIPPED with the reason — never as PASS. |
