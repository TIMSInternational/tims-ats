...
- **`prisma migrate deploy` must NEVER be used against production.** That is a forward-looking rule, not
  a claim about the past — keep the two separate. Prod has no `_prisma_migrations` table —
