...
| 6.10 | `candidate.addToPool` | mutation | `{ candidateId, poolType }` | `{ success }` | candidate:update | Add to talent pool |
...
| 6.17 | `candidate.getPoolStats` | query | — | `{ active, passive, historic, internal, exEmployee, total }` | candidate:read | Talent pool KPIs |
| 6.18 | `candidate.bulkTag` | mutation | `{ candidateIds[], tag }` | `{ tagged }` | candidate:update | Bulk tag |
| 6.19 | `candidate.export` | query | `{ filters?, format }` | `{ url }` | candidate:export | Export list |
