...
## Auth flow
1. `/careers/[orgSlug]/login` — email → `supabase.auth.signInWithOtp({ email, emailRedirectTo: …/me })`.
...
3. `/careers/[orgSlug]/me` (server component): verify session; resolve `Candidate{orgId,email}`; absent →
   friendly "no applications for this email at {org}" + sign-out. Else render dashboard.
4. Middleware: allow `/careers/*/login` + `/careers/*/me`; `/me` requires a session but must **not** redirect
...
1. **Candidate session** — `candidateProcedure` + login page + OTP + `/me` shell. *Security-sensitive → fresh review.*
