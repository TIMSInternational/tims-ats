...
- **Phase 5 — strangler (migrate working domains one at a time) — IN PROGRESS.** Per
  ...
  - **First C# prod deploy** — AWS App Runner (Terraform IaC, PR #157; Gate-G3 runbook, PRs #154/#174).
    _(Uncertain / not independently verified from the repo alone: whether this deployment is currently
    receiving ANY live or canary traffic on any route, or is purely a deployed-but-fully-dark target. Treat
    as unconfirmed until Federico states otherwise.)_
...
| Owner          | Item                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| -------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Federico       | **Per-domain production ownership flip** — 11 Phase-5 domains (external-vendor, billing invoice/usage/webhook, reporting-analytics, team-intel, evaluation360, succession, compensation, nine-box, engagement, DEI, FX-gateway) are built + parity-verified + FLIP-READY per `table-ownership.md`, but dark. Flipping a domain (dark → canary → full via `scripts/parity/cli.ts verify <surface>`), then moving its tables to `efcore` in the ledger and deleting the TS logic, is a manual, per-domain, canary-gated decision. **None has been flipped in prod as of this truth-up.** |
