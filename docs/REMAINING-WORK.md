...
- **Phase 5 — strangler (migrate working domains one at a time) — IN PROGRESS.** Per
  ...
  - **FE/TS dark-cutover wrapper layer (2026-07-27, PRs #197–#212) — now fully COMPLETE, both reads and
    ...
    type — **except billing's invoice-read** (`billing.listInvoices`/`billing.getInvoice`,
    ...
    `NEXT_PUBLIC_*_VIA_CSHARP` flag) confirmed this tenant-scoped pair has **zero real FE consumers** — the
    tenant `settings/billing` page only calls `getBillingConfig`/`getCurrentPlan`/`getUsage` (already
    wrapped via `NEXT_PUBLIC_BILLING_USAGE_VIA_CSHARP`), and the only invoice UI in `apps/web`
    (`app/(admin)/platform/invoices/*`) calls the unrelated cross-org `platform.listInvoices`/
    `platform.getInvoice` admin procedures, not this pair. Same "would be dead code" reasoning as
    access-review above and as the write-side exceptions below — intentionally left unwrapped, not a gap.
    "Billing" in the READS list above means its other 3 reads (usage/plan/config) only. WRITES — all 8
