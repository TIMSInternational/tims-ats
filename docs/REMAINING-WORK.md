...
dashboard.ts` wrapper shipped 2026-08-17 behind `NEXT_PUBLIC_DASHBOARD_READ_VIA_CSHARP` (twelve
    ...
    counts FUTURE-dated subscriptions in every one of its twelve months (a real TS defect, reproduced
    rather than fixed, and `getMrrForecast` does not share it); (c) the mapped-column sibling of PR 1's
    TRAP 10 — Npgsql REJECTS a `Kind=Utc` bound against a `timestamp without time zone` column, so
    application-layer UTC instants must be re-kinded at the repository boundary. See
    `docs/architecture/csharp-migration/phase-5-slice-23-pr2-dashboard-insights.md`.
    parity/RLS/RBAC for each surface against the real Supabase prod DB before any flag flips.
  - **Slice 24 (#90) SHIPPED the FIT-engine port — steps 1–3 + step 4's BACKEND half, dark 2026-08-18** — all 6 procedures of
    `fit-engine.ts` behind `Platform:FitEngineReadEnabled` (4 reads) + `Platform:FitEngineWriteEnabled`
    (2 writes), both default false. `explainFit` reproduces every pre-LLM observable (gate →
    assertScoped('vacancy') → fetch → null ⇒ the TS 404) then answers **501** — the narrative needs the
    TS-only `invokeAgent` Bedrock pipeline (team-intel honest-stub precedent). The pure scoring kernels
    are golden-fixtured CROSS-STACK (`contracts/fit-engine-fixtures/`, generated by running the REAL TS
    exports; `tests/fit-engine/kernels-fixtures.test.ts` + `FitEngineKernelsFixtureTests` assert the same
    cases), including a JS-faithful `Math.round` (half toward +∞ — neither `AwayFromZero` nor
    `Floor(x+0.5)` matches on both −12.5 and 0.49999999999999994). Ledger: `job_profiles` +
    `ai_interview_sessions` → `efcoreReadOnly[]`; `fit_scores` + `role_family_weight_profiles` →
    `efcoreStranglerWrite[]` (both upserts are atomic raw `ON CONFLICT`, the same statement Prisma
    emits). Step 5 is **unrunnable by anyone yet**: the parity registry has no fit-engine surface — the
    seed lacks `fit_engine` grants and fit_scores fixtures, so the 6 routes sit on the governance
    allowlist (two documented groups) pending #195-pattern fixture-first registration, tracked on #90.
    Steps 6–7 (flip, TS delete) follow step 5. **Step 4's OTHER half — the
    `apps/web/lib/platform-api/fit-engine.ts` wrapper — is NOT in this slice** (#90's step 4 names it
    explicitly; the same half-step conflation a panel corrected on #81). All six procedures have live
...
| Priority                   | Feature                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| -------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| HIGH (core differentiator) | **Assessment Player** (`docs/WAVE-1.5a-ASSESSMENT-PLAYER.md`, approved, slice 1 authoring shipped #65). Slices 2–4 shipped: candidate take-flow backend + auto-scoring (Slice 2), player UI (Slice 3), dashboard entry point (Slice 4). **Local/dynamic norm scoring shipped** (Slice 5, `docs/superpowers/specs/2026-08-01-assessment-player-norm-scoring-design.md`): `AssessmentResult` now carries `band`/`percentile`/`normSampleSize`, computed live in `submitAssessment` against each org's own per-assessment-type population (plus a one-time backfill for pre-existing results), gated behind a minimum sample size. This is a **dynamic, self-referential norm** (candidates vs. other candidates in the same org), not a static externally-validated reference norm — that gap (fixed cutoffs from a published psychometric battery) is still open, see the LIA row below. + **Wave 1.5b proctoring beta slice** (candidate preflight/monitoring and staff review implemented through .NET 10 behind a default-off flag; real-device accuracy study, retention/erasure, approved consent notice, and staging candidate-to-reviewer validation remain before external beta; see `docs/proctoring-beta.md`). + Wave 3 `assessment-evaluator` agent for essay scoring (lights up `assessment.getExplainability`, currently honest 501). |
...
- **ExchangeRate-API attribution + subprocessor registration — NOW DUE, the precondition already fired**: the C# FX gateway (`ExchangeRateApiGateway`, `services/Tims.Platform/src/Tims.Infrastructure/Fx/`) uses ExchangeRate-API's free/open tier, whose ToS requires (a) attribution — a link to https://www.exchangerate-api.com somewhere in the product once its rates render converted amounts in the UI — and (b) registering it as a data subprocessor (SOC2). This bullet originally deferred both to "before/when the FX flip goes live"; **`Platform__FxReadsEnabled=true` was confirmed live on the running App Runner service 2026-08-15**, so if the FE renders those converted amounts, the obligation is current, not future. Neither is done. See `docs/architecture/csharp-migration/fx-provider-swap-2026-07-28.md`.
- **FX pins were FROZEN at as_of 2026-07-31 while the FX reads served live traffic** (found + fixed-in-repo 2026-08-15): the daily `FxRefreshJob` lives in `Tims.Workers`, which was never deployed, so the live compensation/pay-equity conversions drifted ~2.4% (COP) in fifteen days — and for four of the six endpoints (getBandDistribution, getTotalCompBreakdown, compensation getDashboardKpis, dei.getPayEquity) the TS fallback is deleted outright. Fixed by `FxRefreshHostedService` — the same refresh use case hosted in the DEPLOYED API behind `Platform:FxRefreshEnabled` (default false, dark) — plus `invoices` joining the currency-discovery union (it was missing, so dashboard-read currencies were pinned only by coincidence). **Federico**: flip per `docs/architecture/csharp-migration/fx-refresh-api-host-2026-08-15.md` §Runbook (option A = re-run FxSeedOnce for freshness today; option B = the flag, permanent).
